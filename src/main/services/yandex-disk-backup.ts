...
export class YandexDiskBackup {
  ...
  /**
   * Runs a backup for the given game right now and lets failures propagate
   * to the caller — used by the manual "Backup now" IPC handler, so the UI
   * can actually tell success from failure. backupOnGameExit() above wraps
   * the same logic but swallows errors, since it's a fire-and-forget call
   * on game exit and must never disrupt normal gameplay.
   */
  public static async backupNow(game: Game): Promise<void> {
    await this.performBackup(game);
  }

  private static async performBackup(game: Game): Promise<void> {
    const prefs = await this.getPrefs();
    if (!prefs?.yandexDiskBackupEnabled || !prefs?.yandexDiskToken) return;

    const token = prefs.yandexDiskToken;
    const maxBackups = prefs.yandexDiskMaxBackups ?? 3;
    const tmpDir = os.tmpdir();

    // Unique per-call local file prefix — includes shop+objectId+high-res
    // timestamp so two backups (different games, or the same game backed
    // up twice in quick succession) never collide on the same temp file,
    // even when they land in the same wall-clock second.
    const localFilePrefix = `hydra-ydisk-${game.shop}-${game.objectId}-${Date.now()}`;

    // Run Ludusavi backup to a temp directory
    const ludusaviTmpPath = path.join(tmpDir, localFilePrefix);
    ...
    const metadata = BackupMetadataService.createMetadata(game, ludusaviResult);
    BackupMetadataService.writeMetadataToBackupDir(ludusaviTmpPath, metadata);

    // Create tar.gz — the remote-facing name stays timestamp-based (it's
    // just a display name in the backup manager), but the local temp path
    // is always the unique prefix above.
    const timestamp = new Date()
      .toISOString()
      .replace(/:/g, "-")
      .replace(/\..+/, "");
    const archiveName = `backup_${timestamp}.tar.gz`;
    const archivePath = path.join(tmpDir, `${localFilePrefix}.tar.gz`);
    ...
    try {
      const sidecarPath = path.join(tmpDir, `${localFilePrefix}.metadata.json`);
      fs.writeFileSync(sidecarPath, JSON.stringify(metadata, null, 2), "utf-8");
      await this.uploadFile(token, sidecarPath, `${remotePath}.metadata.json`);
    ...
    // Prune old backups — deleteBackupFile() removes both the archive and
    // its metadata sidecar, so retention doesn't leave orphaned sidecars
    // behind eating into the user's Yandex Disk quota.
    const backups = await this.listBackups(game);
    if (backups.length > maxBackups) {
      const toDelete = backups.slice(maxBackups);
      for (const entry of toDelete) {
        try {
          await this.deleteBackupFile(entry.path);
        } catch (e) {
