...
export class BackupMetadataService {
  ...
  private static findBestMatchingDir(
    ...
    // Ambiguous: multiple current save directories exist and none shares
    // the original leaf name. Guessing here risks merging files from
    // distinct save trees into the wrong directory (overwriting unrelated
    // saves), so we skip relocation for this base dir rather than guess.
    logger.warn(
      `[BackupMetadata] Ambiguous relocation target for "${originalDir}" among ` +
        `${candidates.length} candidates — skipping relocation for this directory`
    );
    return null;
  }
}
